...
=over 4
    ...
    ...
    # start reading
    ./bin/changelog-run -db=db.sqlite -s=/path/to/changelog
    ...
    # make new driver module
    ./bin/changelog-run -md --author='Mario Zieschang' -email='mziescha@cpan.org' -driver=MySQL -dir=/path/to/directory
    ...
    # make new file read module
    ./bin/changelog-run -mf -a='Mario Zieschang' -@='mziescha@cpan.org' -t=XML -dir=/path/to/directory
    ...
...
# Encapsulated class data.
{
    ...
    sub _start {
        ...
        $config->{dir} = $config->{start};
        my $dbi    = "dbi:$config->{driver}:database=$config->{db}";
        my $dbh    = ( defined $config->{user} ) ? DBI->connect( $dbi, $config->{user}, $config->{pass} ) : DBI->connect($dbi);
        my $insert = {
            dbh       => $dbh,
            db_driver => $config->{driver}
        };
...
sub run {
    ...
    if ( $config{makedriver} ) {
        ...
        my $set = DBIx::Schema::Changelog::Command::Changeset->new( dir => $config{dir} || '.' );
        $set->make( \%config );
    }
    elsif ( defined $config{start} ) { $self->_start( \%config ); }
    elsif ( $config{help} ) { pod2usage( { -verbose => 1 } ); }
